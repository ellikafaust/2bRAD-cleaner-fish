...
Prints out list of calls to trim2bRAD_dedup.pl, one for
each fastq file, to to trim and deduplicate raw 2bRAD reads.
See help for trim2bRAD_dedup.pl for more details.
prints to STDOUT
Arguments:
...
foreach $fqf (@fqs) {
	...
	print "trim2bRAD_dedup.pl input=$fqf site=\"$site\" adaptor=\"$adaptor\" sampleID=$sampleid deduplicate=1 \> $outname\n";
}